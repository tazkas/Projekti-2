<?php 
	...
	*/
	...
	// Device page displays the maint content of the page
	...
	if(isset($_GET["model"])) {
		...
		if($result = mysqli_query($conn, $query)) {
			if(mysqli_num_rows($result) > 0) {
				...
				$device_page .= "<div class='col-md-8'><label>Valitse laitteen omistaja</label><select class='form-control'>";
				foreach($owner_arr as $owner) {
					$device_page .= "<option>$owner[name] | Saatavilla " . $owner['count']['free'] . " kpl | Yhteensä " . $owner['count']['total'] . " kpl</option>\n";
				}
				...
				if($result = mysqli_query($conn, $query)){
					...
					echo "<div class='col-md-8'><br/><label>Kuvaus</label>\n<br/><textarea readonly style='border:none;resize:none;width:100%;height:167px;background-color:#f4f7fc;padding:20px;border-radius:25px;'>$row[description]</textarea><br/><br/></div>";
				}
				/*rows='7' cols='90'*/
				// Datepicker and reservation
				echo "<div class='row'><div class='col-md-8' style='overflow-y:hidden;'><form class='form-inline'> <p>Aloitus: &nbsp;&nbsp;<input type='text' class='datepicker form-control'> Klo: <select type='text' class='klo' id='klo1'>";
				...
				echo $hours;
				echo "</select></p><p>Lopetus: <input type='text' class='datepicker form-control'> Klo: <select type='text' class='klo' id='klo2'>";
				echo "$hours\n</select></p></form></div>"; // End of datepicker col

				echo "<div class='col-md-4'><br/><br/><input class='form-control btn-success' type='button' value='Alustava varaus' onclick=''></div>"; // Reservation col

				echo "</div><br/><br/><br/>"; // End of row
