<?php
	...
	*/
	...
	$query = "select id, category_name from $table_category";
	if($result = mysqli_query($conn,$query)){
		while($row = mysqli_fetch_assoc($result)){
			$category_options .= "<option value='$row[id]'>$row[category_name]</option>\n";
			$existing_categories[] = $row['category_name'];
		}
	}
	...
	if(isset($_POST["save-category"])){
		...
		if(!in_array($category_name, $existing_categories)){
			...
			if($ok == true){
				$query = "insert into category (image,category_name) values('$category_img', '$category_name')";
				if($result = mysqli_query($conn, $query))
	...
	elseif(isset($_POST["device-save"])){
		...
		if($ok == true){
			...
			if($ok == true){
				...
				foreach($eans as $ean)
					$query .= "('$ean','$model', '$type','$category','$description','$device_img', '3', 'VAPAA', '$device_location'),";
				$query = substr($query, 0, strlen($query)-1);
				if($result = mysqli_query($conn, $query))
					echo "<div class='alert alert-success'><strong>Laitteen lisäys onnistui!</strong></div>";
				else
					echo "<div class='alert alert-danger'><strong>Virhe! Kantaan lisääminen epäonnistui!</strong></div>" . mysqli_error($conn);
			}
		}
		else{
...
<div class='row'>
	<div class='col-md-8 col-xs-12'>
		...
		<br/>
		<button type='button' class='btn btn-info' data-toggle='collapse' data-target='#new-device' id='new-device-btn'>
			Lisää uusi laite
		</button>
		<br/><br/>
		<button type='button' class='btn btn-warning' data-toggle='collapse' data-target='#edit-device' id='edit-device-btn'>
			Muokkaa laitetta
		</button>
		<br/><br/>
		<button type='button' class='btn btn-danger' data-toggle='collapse' data-target='#delete-device' id='delete-device-btn'>
			Poista laite
		</button>
		<br/><br/>
		<div id='new-device' class='collapse'>
			...
			<form action='' method='POST' enctype='multipart/form-data' id='device-new'>
				...
				<br/>
				<div class='form-group'>
				    <label for='device-type'>Laitteen tagit:</label>
